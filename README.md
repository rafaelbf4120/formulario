...
    // Monta filtros (se as datas existirem)
    let filtros = [];
...
        console.log("Dados recebidos do Firestore:", allEntries); 
...
        // Define a ordem e os cabeçalhos bonitos para o CSV
        const orderedKeys = [
            'motorista', 'matricula', 'transportado', 'data', 'origem', 
            'partida', 'destino', 'chegada', 'valor', 'valorExtra', 'observacao'
        ];

        const headers = [
            'Motorista', 'Matrícula', 'Transportado', 'Data', 'Origem', 
            'Partida', 'Destino', 'Chegada', 'Valor (R$)', 'Valor Extra (R$)', 'Observação'
        ];

        // Mapeia os dados para linhas CSV
        const rows = allEntries.map(obj => orderedKeys.map(key => {
            let value = obj[key] ?? '';
            // Formata valores de moeda
            if (key === 'valor' || key === 'valorExtra') {
                // Garante que o valor é um número (ou 0 se for nulo) e o formata para BRL
                value = parseFloat(value || 0).toFixed(2).replace('.', ',');
            }
            // Envolve o valor em aspas para tratar vírgulas/pontos e evita problemas de quebra de linha
            return `"${value.toString().replace(/"/g, '""')}"`; 
        }).join(';')); // Usa ponto e vírgula como delimitador (padrão brasileiro)

        // Cria o conteúdo final do CSV
        const csvContent = `${headers.join(';')}\n${rows.join('\n')}`;
        const bom = '\uFEFF'; // Byte Order Mark para compatibilidade com acentos no Excel
